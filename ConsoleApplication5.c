...
/*
Використовуючи метод «бульбашки», впорядкувати по зростанню
значення, що містяться в рядках двовимірного масиву C з робочим розміром
...
int main(void)
{
	 ...
	 *Якщо користувач введе такі n та m які в добутку дадуть значення більше або рівне MAX_ARRAY_SIZE то програма попросить підтвердження.
	 *Якщо користувач введе 1 то значення будуть підтверджені якщо 2 то користувача перекине на повторний ввід n та m.
	...
	while (!inputControlGeneral)
	{
		while (!controlInput_n)
		{
			...
			{
				printf("Довжина рядка не може бути від'ємною, символьною чи більше цього %d значення\n", INT_MAX);
				scanf("%*[^\n]");
			}
			else
			{
		...
		while (!controlInput_m)
		{
			...
			{
				printf("Довжина стовпця не може бути від'ємною, символьною чи більше цього %d значення\n", INT_MAX);
				scanf("%*[^\n]");
			}
			else
			{
